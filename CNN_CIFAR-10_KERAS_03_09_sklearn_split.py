...
cnn.add(Dense(128, activation='relu', kernel_initializer=He_normal))        #은닉층, 150개의 노드를 지니며 활성함수 'relu' 따라서 은닉층 5개의 구조이다
cnn.add(Dropout(0.2))
...
#영상 증대기 활용
generator=ImageDataGenerator(rotation_range=10.0, width_shift_range=0.2, height_shift_range=0.2, horizontal_flip=True)
# gen=generator.flow(x_train_normalized, y_train, batch_size=batch_siz)
...
# 모델 학습하기
history=cnn.fit_generator(generator.flow(x_train_images, y_train_labels, batch_size=128), epochs=150, validation_data=(x_val, y_val))
